		...
		<div id="content">
			...
			<?php
				...
				if(!empty($IdMuridNow)){
					...
					$result3 = mysqli_query($conn, "SELECT count(Kebenaran) AS 'betul' FROM keputusan WHERE IdMurid='$IdMuridNow' AND Kebenaran='betul'");
					$row3 = mysqli_fetch_array($result3);
					$betul = $row3['betul'];

					// Calculate total number of questions which are answered by the specific student
					$result4 = mysqli_query($conn, "SELECT count(Kebenaran) AS 'totalSoalan' FROM keputusan WHERE IdMurid='$IdMuridNow'");
